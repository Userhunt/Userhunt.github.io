...
function generateComponent(data, struct, options) {
  ...
  switch (struct.type) {
    case 'systemField': return generateSystemField(data, struct, options);
    case 'string': return generateString(data, struct, options);
    case 'scoreboard': return generateSystemField(data, struct, options);
    case 'count': return generateString(data, struct, options);
    case 'int': return generateString(data, struct, options);
    case 'id': return generateId(data, struct, options);
...
function generateId(data, struct) {
  ...
  if (data && !(data.indexOf('minecraft:')==0 || data.substring(0,1)=='#')) {
    $el["0"].children[1].classList.add("error-form-control")
  }
  return {out: data, component: $el};
}

function generateSystemField(data, struct) {
  let $el = $('#components').find('[data-type="string"]').clone();
  $el.attr('data-type', struct.type);
  $el.attr('data-index', struct.id);
  $el.attr('data-resource', struct.resource);
...
function generateMap(data, struct) {
  ...
  $el.find('[data-name="3"]').attr('data-i18n', "generate");
  if (struct.help) {
